...
class UserManagerService {
    constructor(fs, repository) {
        ...
        this.cachedUsers = [];
        this.cachedGroups = [];
        this.lastUsersSync = -1;
        this.lastShadowSync = -1; // 🌟 Nueva marca para trackear /etc/shadow
        this.lastGroupsSync = -1;
    }
    ...
    updatePassword(username, clearTextPassword) {
        this.refreshUsers();
    ...
    deleteUser(username) {
        ...
        this.refreshGroups();
        if (username === 'root')
            return "deluser: cannot remove root";
        if (!this.cachedUsers.some(u => u.username === username))
