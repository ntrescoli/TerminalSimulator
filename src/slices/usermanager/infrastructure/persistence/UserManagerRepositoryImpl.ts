...
export class UserManagerRepositoryImpl implements IUserManagerRepository {
  ...
  public saveUsers(users: User[]): void {
    const content = users
      .map(
        (u) =>
          `${u.username}:x:${u.uid}:${u.gid}:${u.fullName}:${u.home}:${u.shell}`,
      )
      .join("\n");
    ...
    const currentShadowMap = currentShadow.isSuccess
      ? this.parseShadow(currentShadow.getValue())
    ...
    const shadowContent =
      users
        .map((u) => {
          ...
          const hash =
            u.password ||
            currentShadowMap.get(u.username) ||
            "$6$rounds=5000$jsTerminalSalt$c37ce20fffffffff";
